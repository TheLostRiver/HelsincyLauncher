...
Phase 136 - Backoff Policy Boundary (complete)
...
- AT-2026-05-19-265 completed the backoff policy, retry exhaustion, user-attention, and terminal failure eligibility boundary before any Rust retry policy implementation.
...
### Phase 107: One-shot Queue Policy Slot Gate

- Outcome: implement the documented `kernel-jobs` one-shot policy gate so queued dispatch defers when current `Running` snapshots have exhausted `RuntimeQueuePolicy::max_concurrent_jobs`, while preserving deterministic selection when capacity remains.
- Atomic tasks: AT-2026-05-17-232
- **Status:** complete

### Phase 108: Composition One-shot Runtime Execution Helper Boundary

- Outcome: document the next composition-root boundary for explicitly composing `SharedJobRuntimeHost` and `JobDriverRegistry<()>` into a one-shot execution helper, without adding automatic scheduler loops, hidden startup work, durable leases, terminal projection, downloads IO, transport, frontend, or SQLite schema changes.
- Atomic tasks: AT-2026-05-17-233
- **Status:** complete

### Phase 109: Composition One-shot Runtime Execution Helper

- Outcome: implement the documented composition-root one-shot runtime execution helper and wire it through `build_startup_pipeline(...)` without invoking it automatically from construction or startup stages.
...
- Outcome: implement the documented `jobs_run_next_execution_turn` host command and stable runtime execution-turn DTO with a RED/GREEN transport smoke path, without touching frontend, runtime internals, composition helper behavior, downloads business logic, scheduler loops, or schema.
...
### Phase 116: Downloads Guarded Writer Port

- Outcome: implement the documented guarded writer wrapper behind `DownloadSegmentWritePort`, preserving safe delegation and mapping unsafe staging targets into existing handled write failures without real disk IO or production wiring.
...
### Phase 121: Downloads Segment Length Verifier Boundary

- Outcome: document the first concrete verifier shell behind `DownloadSegmentVerifyPort`, starting with byte-length verification and preserving hash, retry/backoff, public `DL_*` projection, production wiring, host transport, frontend, and schema work for later slices.
...
### Phase 123: Downloads Length Verifier Partial Resume Semantics

- Outcome: correct the concrete length verifier so from-start requests compare current written bytes to total segment length, while partial requests compare `start_offset + current written bytes` to total segment length.
- Atomic tasks: AT-2026-05-19-252
- **Status:** complete

### Phase 124: Downloads Static Segment Fetcher Boundary

- Outcome: document the first concrete fetcher boundary behind `DownloadSegmentFetchPort`, starting with deterministic static/local byte sources and preserving real HTTP range, provider auth, retry/backoff, public network/provider errors, production wiring, host transport, frontend, and schema work for later slices.
- Atomic tasks: AT-2026-05-19-253
- **Status:** complete

### Phase 125: Downloads Static Segment Fetcher

- Outcome: implement `DownloadSegmentStaticFetchPort` behind `DownloadSegmentFetchPort`, proving from-start bytes/etag, partial remaining-byte fetches, handled missing/invalid source failures, and executor completion without real HTTP/provider behavior.
...
### Phase 131: Downloads Driver Completion-first Terminal Implementation

- Outcome: make `DownloadJobDriver::run(...)` return `JobRunDisposition::Completed` for a saved non-empty all-completed known checkpoint, while keeping failed segment mutation non-terminal.
...
### Phase 133: Failed Segment Metadata Persistence

- Outcome: persist module-local failed segment diagnostic reason and retryable hint through downloads checkpoint records and SQLite adapter mapping.
- Atomic tasks: AT-2026-05-19-262
- **Status:** complete

### Phase 134: Retry Backoff Failure-Class Boundary

- Outcome: define durable retry count, backoff eligibility, and module-owned failure classes before implementing Rust retry policy fields.
- Atomic tasks: AT-2026-05-19-263
- **Status:** complete
