...
按当前仓库注释规范，在不改动任何运行时行为或已有正确英文注释的前提下，为 SQLite storage adapter 中共享作业快照存储外壳簇的缺失声明级中文注释补齐：

- `crates/adapter-storage-sqlite/src/lib.rs`

本轮只补 `crates/adapter-storage-sqlite/src/lib.rs` 中 `SqliteJobSnapshotStore` 的公开声明注释，不改当前 snapshot 序列化、恢复查询、表结构、锁使用或其他仓储行为，也不顺带打开第二个源码文件。
...
- out of scope:
  ...
  - annotate more than this one snapshot-store declaration cluster in this file
  - change database path handling, `rusqlite` usage, schema initialization, or repository behavior
  - change snapshot upsert/get/list_resumable behavior, table initialization logic, schema shape, or lock semantics
...
- falsifiable local hypothesis: If `crates/adapter-storage-sqlite/src/lib.rs` adds Chinese declaration comments for the public `SqliteJobSnapshotStore` shell while leaving its current SQLite connection ownership, snapshot persistence behavior, and recovery query logic unchanged, then this shared runtime snapshot-store slice will satisfy the repository comment rule and the job-runtime/storage boundary guidance without changing runtime behavior.
...
- `SqliteJobSnapshotStore` is the strongest next missing-comment boundary because it is the next remaining public declaration cluster in the same file and directly owns the shared runtime snapshot persistence boundary defined by the kernel-jobs runtime design.
- This slice stays at declaration level only; no snapshot persistence semantics, schema initialization logic, lock usage, or recovery query behavior are changed.
- `cargo check -p launcher-adapter-storage-sqlite --manifest-path q:\DEV\MyEpicLauncher\Cargo.toml --lib` is the narrowest current executable validation gate for this job-snapshot-store slice because the crate exposes no narrower named test anchor and this check compiles the touched public shell surface.
...
- AT-2026-05-08-097 has been validated and is ready for selective publication.
