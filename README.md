...
5. downloads 后端正在按模块实现文档推进 concrete segment execution：filesystem writer、length verifier、static fetcher 已完成，下一步是 composition-root 的执行器接线边界。
...
2. downloads 后端优先完成 composition-root execution-port wiring，再进入 runtime terminal projection、retry/backoff 和稳定 `DL_*` 执行错误分类。
