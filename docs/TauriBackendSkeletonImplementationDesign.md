...
| B2 | `crates/kernel-foundation/Cargo.toml`, `crates/kernel-foundation/src/lib.rs`, `error.rs`, `result.rs`, `clock.rs`, `ids.rs`, `paging.rs`, `time.rs`, `crates/kernel-foundation/tests/foundation_contract_smoke.rs` | 补齐最小基础类型和导出面，并落一条明确存在的 foundation smoke test | `cargo test -p launcher-kernel-foundation foundation_contract_smoke` |
